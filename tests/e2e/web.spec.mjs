...
async function mockCartesBackend(page, overrides = {}) {
  ...
  await page.route('**/.netlify/functions/cartes-link', async route => {
    const body = route.request().postDataJSON?.() || {};
    if (body.action === 'status') return route.fulfill({ json: { linked: Boolean(overrides.linked), usage: overrides.usage || { plan: 'gratuito', limite: 5, usadas: 0, disponibles: 5, periodo: '2026-08' } } });
    return route.fulfill({ json: { linked: false, code: '123456', instruction: 'VINCULAR 123456' } });
  });
  await page.route('**/.netlify/functions/guia-masonico', route => route.fulfill({
    json: { answer: 'Respuesta de prueba de Cartes.', usage: { plan: 'gratuito', limite: 5, usadas: 1, disponibles: 4, periodo: '2026-08' } }
  }));
}
...
test('Cartes muestra y actualiza las consultas disponibles', async ({ page }) => {
  await mockCartesBackend(page);
  await page.goto('/');
  await page.locator('.gm-launcher').click();
  await expect(page.locator('.gm-header__usage')).toHaveText('5 consultas disponibles');
  await page.locator('.gm-input').fill('¿Qué representa la escuadra?');
  await page.locator('.gm-send').click();
  await expect(page.locator('.gm-header__usage')).toHaveText('4 consultas disponibles');
});
...
test('limpiar conversación llama a memoria central', async ({ page }) => {
  ...
  await page.route('**/.netlify/functions/cartes-conversation', async route => {
    ...
    return route.fulfill({ json: body.action === 'history' ? { messages: [] } : { ok: true } });
  });
...
test('suscripcion y suscripción muestran estado sin consumir consulta', async ({ page }) => {
  let consultasAlMotor = 0;

  await mockCartesBackend(page, {
    linked: true,
    usage: { plan: 'plus', limite: 50, usadas: 2, disponibles: 48, periodo: '2026-08' }
  });
  ...
  await page.route('**/.netlify/functions/guia-masonico', async route => {
    ...
    return route.fulfill({
      json: {
        ...
        usage: { plan: 'plus', limite: 50, usadas: 3, disponibles: 47, periodo: '2026-08' }
      }
    });
  });
  ...
  for (const comando of ['suscripcion', 'suscripción', 'mi suscripcion', 'mi suscripción']) {
    await page.locator('.gm-input').fill(comando);
    await page.locator('.gm-send').click();

    await expect(page.locator('.gm-messages')).toContainText('Plan actual: Cartes Plus');
    await expect(page.locator('.gm-messages')).toContainText('Estado Cartes Plus: Activo');
    await expect(page.locator('.gm-header__usage')).toHaveText('48 consultas disponibles');
  }
...
test('Vincular ofrece apertura y copia como respaldo en escritorio', async ({ page }) => {
  ...
  await expect(page.locator('.gm-messages')).toContainText('Generé tu código 123456');
