...
function isPublishedByDate(item) {
	const date = String(item && item.fecha ? item.fecha : "").trim();
	if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return true;
	return date <= getSiteDate();
}
...
async function loadEpisodes() {
	try {
		...
		allEpisodes = (Array.isArray(episodesData) ? episodesData : [])
			.filter(isPublishedByDate)
...
function initAjaxForm({ formId, modalId, closeSelector, errorLabel, errorMessage }) {
	...
	form.addEventListener("submit", async (event) => {
		...
		try {
			...
			window.alert(errorMessage || "No se pudo enviar el formulario. Conservamos tus datos para que puedas intentarlo nuevamente.");
		} finally {
			if (submitButton) {
...
function initEpisodeAlertForm() {
	initAjaxForm({
		...
		errorMessage: "No se pudo registrar el aviso. Conservamos tu correo para que puedas intentarlo nuevamente."
	});
}
...
function initSubscribeForm() {
	initAjaxForm({
		...
		errorMessage: "No se pudo completar la suscripción. Conservamos tu correo para que puedas intentarlo nuevamente."
	});
}
...
function initProposalForm() {
	initAjaxForm({
		...
		errorMessage: "No se pudo enviar la propuesta. Conservamos tus datos para que puedas intentarlo nuevamente."
	});
}
...
function initContactForm() {
	initAjaxForm({
		...
		errorMessage: "No se pudo enviar el mensaje. Conservamos tus datos para que puedas intentarlo nuevamente."
	});
}
...
function initUnsubscribeEpisodesForm() {
	initAjaxForm({
		...
		errorMessage: "No se pudo enviar la solicitud de baja. Conservamos tus datos para que puedas intentarlo nuevamente."
	});
}
...
function updateBoletinesLoadMoreButton(total) {
	...
	try {
		...
		const reflexionesData = await res.json();

		allReflexiones = (Array.isArray(reflexionesData) ? reflexionesData : [])
			.filter(isPublishedByDate);

		if (allReflexiones.length === 0) {
...
async function loadNovedades() {
	...
	if (!ticker || !newsbar) return;

	try {
		const response = await fetch("assets/data/novedades.json", {
			cache: "no-store"
		});

		if (!response.ok) {
			throw new Error(`No se pudo cargar novedades.json: ${response.status}`);
		}

		const data = await response.json();
		const novedades = Array.isArray(data) ? data : [];

		const hoy = getSiteDate();

		const activas = novedades.filter((item) => {
			if (!item || item.activo === false || !item.texto) return false;

			const fechaInicio = String(item.fechaInicio || "").trim();
			const fechaFin = String(item.fechaFin || "").trim();

			const inicia = !fechaInicio || fechaInicio <= hoy;
			const finaliza = !fechaFin || fechaFin >= hoy;
		...
		function renderNovedad(item) {
			ticker.replaceChildren();

			const contenido = document.createElement(item.url ? "a" : "span");

			if (item.url) {
				contenido.href = item.url;
				contenido.className = "newsbar__link";
			...
			contenido.textContent = `${item.icono ? `${item.icono} ` : ""}${item.texto}`;
			ticker.appendChild(contenido);
		}

		function mostrarSiguienteNovedad() {
			renderNovedad(activas[indice]);
			indice = (indice + 1) % activas.length;
		}

		mostrarSiguienteNovedad();
...
async function loadCartesPage() {
	const root = document.querySelector("[data-cartes-page]");
	if (!root) return;

	try {
		const response = await fetch("assets/data/cartes.json", { cache: "no-store" });
		if (!response.ok) throw new Error(`No se pudo cargar cartes.json: ${response.status}`);

		const data = await response.json();
		const launchDate = new Date(data.fechaLanzamiento);

		const status = document.getElementById("cartesStatus");
		const ctaHero = document.getElementById("cartesHeroCta");
		const ctaFinal = document.getElementById("cartesFinalCta");
		const finalTitle = document.getElementById("cartesFinalTitle");
		const finalText = document.getElementById("cartesFinalText");
		const countdownSection = document.getElementById("cartesCountdownSection");

		function applyState(isLaunched) {
			const state = isLaunched ? data.lanzamiento : data.preLanzamiento;

			if (status) status.textContent = state.estado;

			[ctaHero, ctaFinal].forEach((cta) => {
				if (!cta) return;
				cta.textContent = state.ctaTexto;
				cta.href = state.ctaUrl;

				if (/^https?:\/\//i.test(state.ctaUrl)) {
					cta.target = "_blank";
					cta.rel = "noopener noreferrer";
				}
			});

			if (finalTitle) finalTitle.textContent = state.tituloFinal;
			if (finalText) finalText.textContent = state.textoFinal;
			if (countdownSection) countdownSection.hidden = isLaunched;
		}
		...
		function updateCountdown() {
			...
			const values = {
				cartesDays: Math.floor(distance / 86400000),
				cartesHours: Math.floor((distance % 86400000) / 3600000),
				cartesMinutes: Math.floor((distance % 3600000) / 60000),
				cartesSeconds: Math.floor((distance % 60000) / 1000)
			};
		...
		console.error("Error cargando la configuración de Cartes:", error);
	}
}
